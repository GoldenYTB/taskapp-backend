...
const MIN_WD = parseFloat(process.env.MIN_WITHDRAWAL_USD || "1.00");
...
// In the Adsgram dashboard set your "Reward URL" to:
//   https://YOUR-BACKEND/postback/adsgram?userid={userid}&txn={transaction_id}&sig=<sha256(userid:txn:SECRET)>
// Adsgram pays a fixed CPM; set ADSGRAM_REWARD_USD to your average revenue per completed view.
const ADSGRAM_REWARD_USD = parseFloat(process.env.ADSGRAM_REWARD_USD || "0.01");

app.get("/postback/adsgram", (req, res) => {
  const { userid, txn, sig } = req.query;
  if (!userid || !txn) return res.status(400).send("missing params");

  const expected = crypto
    .createHash("sha256")
    .update(`${userid}:${txn}:${SECRET}`)
    .digest("hex");
  if (sig !== expected) return res.status(403).send("bad signature");
  ...
  try {
    ...
    return res.status(200).send("duplicate ignored"); // replay-safe via UNIQUE(network,txn)
  }
